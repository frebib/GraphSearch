 ...
 *
 * (1) The class Nil. (2) The class Cons.
 *
 * There is only one object of type NIL, the empty list.
 *
 * An object of type Cons<E> has a head, of type E, and a tail, which is a
 * (reference to an) IList<E>.
 *
 * A list [1,2,3] is written (rather verbosely) as
 *
 * new Cons<Integer>(1,new Cons<Integer>(2,new Cons<Integer>(3,new
 * Nil<Integer>())));
 *
 ...
 *
 * This is an instance of the "composite pattern", because we have one interface
 * for lists, but two class implementations, one for each case Nil and Cons. Any
 * list is built by combining Nil and Cons as above.
 *

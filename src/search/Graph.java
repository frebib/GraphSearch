...
public class Graph<A> {
	...
	public static void main(String args[]) {
		...
		for (int[] element : nick) {
			// What we are going to do relies on the two following facts
			// about nick:
			assert (element.length >= 2);       // (1)
			assert (element.length % 2 == 0);   // (2)
			...
			for (int j = 2; j < element.length; j = j + 2) {
				int sx = element[j];
				int sy = element[j + 1];
		...
		Predicate<Coordinate> p1 = new Predicate<Coordinate>() {
			@Override
			public boolean holds(Coordinate a) {
				return a == end.getContents();
			}
		};

		Predicate<Coordinate> p2 = new Predicate<Coordinate>() {
			@Override
			public boolean holds(Coordinate a) {
				return a == inaccessableNode.getContents();
			}
		};
